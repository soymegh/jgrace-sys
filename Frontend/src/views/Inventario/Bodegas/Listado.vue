<template>
    <b-card>
        <b-row>
            <div class="col-md-3 sm-text-center">
                <router-link class="btn btn-success" tag="button"
                             :to="{ name: 'inventario-bodegas-registrar' }">
                    <feather-icon icon="PlusCircleIcon"></feather-icon>
                    Registrar
                </router-link>
            </div>

            <div @keyup.enter="filter.page = 1;obtenerBodegas();"
                ...
                <input v-model="filter.search.value" class="form-control mb-1 mr-sm-1 mb-sm-0" placeholder="Buscar"
                       type="text">
                <b-button variant="info" @click="filter.page = 1;obtenerBodegas();"
            ...
            <table class="table table-striped table-bordered">
                ...
                <template v-for="bodega in bodegas">
                    <tr :key="bodega.id_bodega">
                        <td>{{ bodega.tipo_bodega.descripcion }}</td>
                        <td>{{ bodega.sucursal_bodega.descripcion}}</td>
                        <td>{{bodega.descripcion}}</td>
                        <!--<template v-if="bodega.centro_costo_bodega">
                            <td>{{bodega.centro_costo.descripcion}}</td>
                        </template>
                        <template v-else>
                            <td>N/A</td>
                        </template> -->
                        <template v-if="bodega.permite_venta">
                        ...
                        <td class="text-center">
                            <template v-if="[1].indexOf(bodega.id_bodega) < 0">
                                <router-link v-b-tooltip="'Actualizar Bodega'"
                                             :to="{name: 'inventario-bodegas-actualizar', params: {id_bodega: bodega.id_bodega}}"
                                             tag="a">
        ...
        <b-card-footer>
            <pagination @changePage="changePage" @changeLimit="changeLimit" :items="bodegas" :total="total"
