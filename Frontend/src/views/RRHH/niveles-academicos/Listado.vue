	...
	<div class="main">
		<div class="content-box">
			<div class="content-box-header">
				<div class="box-title">Administrar Niveles academicos</div>
				<div class="box-description">Listado de niveles academicos</div>
				<div class="box-description"><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal' }">Módulos</router-link>><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal-nomina' }"> Módulo nomina</router-link>> Niveles academicos</div>
			</div>
			<div class="row">
				<div class="col-md-6 sm-text-center">
					<router-link :to="{ name: 'registrar-niveles-academicos' }" class="btn btn-success" tag="button">
						<i class="pe-7s-plus"></i> Registrar nivel
					</router-link>
				</div>
			...
			<div class="table-responsive mt-3">
				<table class="table table-striped table-bordered">
					...
					<tbody>
						<tr :key="nivel.id_nivel_academico" v-for="nivel in niveles">
							<td>{{ nivel.descripcion }}</td>
							<td>{{ nivel.abreviatura }}</td>
							<td>{{ nivel.acreditacion }}</td>
							...
							<td class="text-center">
								<router-link :to="{ name: 'actualizar-niveles-academicos', params: { id_nivel_academico: nivel.id_nivel_academico } }" tag="a"><i class="icon-pencil"></i></router-link>
							</td>
						</tr>
						<tr v-if="!niveles.length">
							<td class="text-center" colspan="5">Sin datos</td>
						</tr>
					</tbody>
				</table>
			</div>
			<pagination :items="niveles" :limit="filter.limit" :limitOptions="filter.limitOptions" :page="filter.page" :total="total" @changeLimit="changeLimit" @changePage="changePage"></pagination>
...
<script type="text/ecmascript-6">
	import nivel from '../../api/niveles_academicos';
	...
	export default {
		...
		methods: {
			obtener() {
				var self = this
				self.loading = true;
				nivel.obtener(self.filter, data => {
					self.niveles = data.rows
