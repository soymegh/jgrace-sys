	...
	<div class="main">
		<div class="content-box">
			<div class="content-box-header">
				<div class="box-title">Administrar Normativas</div>
				<div class="box-description">Listado de Normativas</div>
				<div class="box-description"><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal' }">Módulos</router-link>><router-link :style="'margin-right: 10px;color: blue;'" :to="{ name: 'pagina-principal-tesoreria' }"> Módulo tesoreria</router-link>> Normativas</div>
			</div>
			<div class="row">
				<div class="col-md-6 sm-text-center">
					<router-link :to="{ name: 'registrar-normativas' }" class="btn btn-success" tag="button">
						<i class="pe-7s-plus"></i> Registrar Normativa
					</router-link>
				</div>
			...
			<div class="table-responsive mt-3">
				<table class="table table-striped table-bordered">
					<thead>
						<tr>
							<th>Concepto</th>
							<th class="text-center table-number">Monto minimo C$</th>
							<th class="text-center table-number">Monto máximo C$</th>
							<th>Regulación de tiempo</th>
							<th class="text-center">Estado</th>
							<th class="text-center action">Opciones</th>
						</tr>
					</thead>
					<tbody>
						<tr :key="normativa.id_normativa" v-for="normativa in normativas">
							<td>{{ normativa.concepto }}</td>
							<td>{{ normativa.monto_minimo }}</td>
							<td>{{ normativa.monto_maximo }}</td>
							<td>{{ normativa.tiempo }}</td>
							<td class="text-center">
							 <div v-if="normativa.estado">
							...
							<td class="text-center">
								<router-link :to="{ name: 'actualizar-normativas', params: { id_normativa: normativa.id_normativa } }" tag="a"><i class="icon-pencil"></i></router-link>
							</td>
						</tr>
						<tr v-if="!normativas.length">
							<td class="text-center" colspan="5">Sin datos</td>
						</tr>
					</tbody>
				</table>
			</div>
			<pagination :items="normativas" :limit="filter.limit" :limitOptions="filter.limitOptions" :page="filter.page" :total="total" @changeLimit="changeLimit" @changePage="changePage"></pagination>
