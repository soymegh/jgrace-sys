        ...
        computed: {
            ...
            total_deuda() {
                ...
                let total_pagos_cordobas = this.form.detallePago.reduce((carry, item) => {
                    return (carry + Number(item.moneda_x.id_moneda === 1 ? Number(item.monto) : Round.round((item.monto_me) * (this.form.tasa_paralela),2)) )
                }, 0);
                ...
                if (Round.round(total_final_cordobas - total_pagos_cordobas,2) < 0) {
                    ...
                    console.log('Pago pendiente mn ' + this.form.pago_pendiente_mn);*/
                    this.form.pago_pendiente = Round.round(this.form.pago_pendiente_mn / this.form.tasa_paralela,2);
                }
            ...
            total_vuelto() {
                ...
                const monto_dolares_decimales = Round.round((Number(this.form.total_final_cordobas) / Number(this.form.tasa_paralela)),2);
                total_pagos = this.form.detallePago.reduce((carry, item) => {
                    ...
                    return carry + Number(item.moneda_x.id_moneda === 1 ? item.monto : Round.round((item.monto_me) * (this.form.tasa_paralela),2))
                }, 0);
                ...
                if (((Round.round((this.form.total_final_cordobas),2) - Round.round((total_pagos),2))) > 0) { /// revision
                    ...
                    this.form.pago_vuelto = Round.round(this.form.pago_vuelto_mn / this.form.tasa_paralela,2);
                }
        ...
        methods: {
          ...
          cerrarModal() {
            ...
            calcular_montos(itemX) {
                ...
                itemX.p_unitario = Round.round(((Number(itemX.subtotal - itemX.mt_descuento - itemX.mt_ajuste) / Number(itemX.cantidad))),2);
