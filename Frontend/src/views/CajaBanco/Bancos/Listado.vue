<template>
    <b-card>
        <b-row>
            <div class="col-md-3 sm-text-center">
                <router-link class="btn btn-success" tag="button"
                             :to="{ name: 'cajabanco-bancos-registrar' }">
                    <feather-icon icon="PlusCircleIcon"></feather-icon>
                    Registrar
                </router-link>
            </div>

            <div @keyup.enter="filter.page = 1;obtenerBancos();"
                ...
                <input v-model="filter.search.value" class="form-control mb-1 mr-sm-1 mb-sm-0" placeholder="Buscar"
                       type="text">
                <b-button variant="info" @click="filter.page = 1;obtenerBancos();"
            ...
            <table class="table table-striped table-bordered">
                ...
                <template v-for="banco in bancos">
                    <tr :key="banco.id_banco">
                        ...
                        <td class="text-center">
                            <template>
                                <router-link v-b-tooltip="'Actualizar Banco'"
                                             :to="{name: 'cajabanco-bancos-actualizar', params: {id_banco: banco.id_banco}}"
                                             tag="a">
        ...
        <b-card-footer>
            <pagination @changePage="changePage" @changeLimit="changeLimit" :items="bancos" :total="total"
