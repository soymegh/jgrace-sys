<template>
    <b-card>
        <b-row>
            <div class="col-md-3 sm-text-center">
                <router-link class="btn btn-success" tag="button"
                             :to="{ name: 'admon-sucursales-registrar' }">
                    <feather-icon icon="PlusCircleIcon"></feather-icon>
                    Registrar
                </router-link>
            </div>

            <div @keyup.enter="filter.page = 1;obtenerSucursales();"
                ...
                <select v-model="filter.search.field" class="form-control mb-1 mr-sm-1 mb-sm-0 search-field">
                    <option value="descripcion">Nombre Sucursal</option>
                    <option value="serie">Serie</option>
                </select>
                <input v-model="filter.search.value" class="form-control mb-1 mr-sm-1 mb-sm-0" placeholder="Buscar"
                       type="text">
                <b-button variant="info" @click="filter.page = 1;obtenerSucursales();"
            ...
            <table class="table table-striped table-bordered">
                ...
                <template v-for="sucursal in sucursales">
                    <tr :key="sucursal.id_sucursal">
                        ...
                        <td>{{ sucursal.serie}}</td>
                        <td>{{sucursal.telefonos}}</td>
                        <td>{{sucursal.direccion}}</td>
                        <td>{{sucursal.numero_autorizacion}}</td>
                        <td class="text-center">
                            <div v-if="sucursal.estado===1">
                        ...
                        <td class="text-center">
                            <template v-if="[1].indexOf(sucursal.id_sucursal) < 0">
                                <router-link v-b-tooltip="'Actualizar Sucursal'"
                                             :to="{name: 'admon-sucursales-actualizar', params: {id_sucursal: sucursal.id_sucursal}}"
                                             tag="a">
        ...
        <b-card-footer>
            <pagination @changePage="changePage" @changeLimit="changeLimit" :items="sucursales" :total="total"
