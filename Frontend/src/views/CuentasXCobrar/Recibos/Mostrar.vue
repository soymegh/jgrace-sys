        ...
        <b-row class="invoice-preview">
            ...
            >
                ...
                >
                        ...
                        <div class="d-flex justify-content-between flex-md-row flex-column invoice-spacing mt-0">
                            ...
                            <div class="mt-md-0 mt-2">
                                <h4 class="invoice-title">
                                    Recibo
                                    <span class="invoice-number">#{{ form.no_documento}}</span>
                                </h4>
                    ...
                    >
                        <b-row class="invoice-spacing">
                            ...
                            <b-col cols="12" xl="4" class="p-0 justify-content-xl-center">
                                ...
                                <router-link
                                        :to="{ name: 'ventas-clientes-actualizar', params: { id_cliente: form.id_cliente } }"
                                        target="_blank">
                                    <p class="card-text mb-25">
                                        {{ form.recibo_cliente.nombre_comercial}}
                                    </p>
                                </router-link>
                                ...
                                <p class="card-text mb-25">
                                    {{form.tipo_recibo === 1 ? 'Cancelación factura' : 'Anticipo de clientes'}}
                                </p>
                            ...
                            <b-col cols="12" xl="4" class="p-0 justify-content-xl-end">
                                ...
                                <div v-if="form.estado===1">
                                    <span class="badge badge-primary" style="font-size: 100%;">Emitido</span>
                                </div>
                                </p>
                                <h6 class="mb-25">
                                    {{ 'Fecha emisión:' }}
                    ...
                    >
                        <template #cell(descripcion_pago)="data">
                            <b-card-text class="font-weight-bold mb-25">
                                {{data.item.descripcion_pago}}
                            </b-card-text>
                        </template>
                        <template #cell(monto_me)="data">
                            <b-card-text class="font-weight-bold mb-25">
                                {{data.item.monto_me| formatMoney(2)}}
                            </b-card-text>
                        </template>


                        <!--Custom footer - summary-->
                        <template #foot(descripcion_pago)="data">
                            <span class="text-bold">{{ 'Totales:' }}</span>
                        </template>
                        <template #foot(monto_me)="data">
                            <span class="text-bold">{{ total_recibo_detalle }}</span>
                        </template>
            ...
            >
                <b-card>
                    ...
                    <b-button
                            ...
                            class="mb-75"
                            block
                            @click.prevent="downloadItem('pdf',form.id_recibo)"
                    >
                        Descargar PDF  <feather-icon icon="DownloadIcon" aria-hidden="true" style="color: #0a91ff"></feather-icon>
                    </b-button>
        ...
        methods: {
            ...
            obtenerRecibo() {
                ...
                recibo.obtenerRecibo(
                    ...
                    data => {
                        self.form = data.recibo;
                        self.nombre_empresa = data.nombre_empresa.valor;
                        self.telefono_empresa = data.telefono_empresa.valor;
                        self.direccion_empresa = data.direccion_empresa.valor;
                        self.items = this.form.recibo_detalles;
                        self.loading = false;
                    },
                    err => {
                        this.$toast({
            ...
            downloadItem(extension, id_recibo){
                const self = this;
                if (!this.descargando) {
                    self.loading = true;
                    let url = 'cuentas-cobrar/Reportes/recibos/' + extension + '/' + id_recibo;
                    ...
                    axios.get(url, {responseType: 'blob'})
                        .then(({data}) => {
                            ...
                            let link = document.createElement('a');
                            link.href = window.URL.createObjectURL(blob)
                            link.download = 'FormatoRecibo.' + extension;
                            link.click()
