...
class RRHHDatosMedicosController extends Controller
{
    /**
     ...
     * @param RRHHDatosMedicos $datos
     * @return  json(array)
     */

    public function obtener(Request $request, RRHHDatosMedicos $datos)
    {
        $datos = $datos->obtenerDatosMedicos($request);
    ...
    public function obtenerTodas(Request $request, RRHHDatosMedicos $datos)
    {
        $datos = RRHHDatosMedicos::orderby('id_dato_medico')->get();
        return response()->json([
            'status' => 'success',
            'result' => $datos,
            'messages' => null
    ...
    public function obtenerDatoMedico(Request $request)
    {
        $rules = [
            'id_dato_medico'=> 'required|integer|min:1'
        ];

        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $datos = RRHHDatosMedicos::find($request->id_dato_medico);
		  ...
		  return response()->json([
				'status' => 'error',
				'result' => array('id_dato_medico'=>["Datos no encontrados"]),
				'messages' => null
			]);
		}
