...
class RRHHSaldosVacacionesController extends Controller
{
    /**
     ...
     * @param RRHHSaldosVacaciones $datos
     * @return  json(array)
     */

    public function obtener(Request $request, RRHHSaldosVacaciones $datos)
    {
        $datos = $datos->obtenerSaldoVacaciones($request);
    ...
    public function obtenerTodas(Request $request, RRHHSaldosVacaciones $datos)
    {
        $datos = RRHHSaldosVacaciones::orderby('id_vacacion_saldo')->with('saldoEmpleado')->first();
        return response()->json([
            'status' => 'success',
            'result' => $datos,
            'messages' => null
    ...
    {
        $rules = [
            'id_vacacion_saldo'=> 'required|integer|min:1'
        ];

        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $datos = RRHHSaldosVacaciones::find($request->id_vacacion_saldo);
		  ...
		  return response()->json([
				'status' => 'error',
				'result' => array('id_parentesco'=>["Datos no encontrados"]),
				'messages' => null
			]);
		}
