...
class RRHHParentescoController extends Controller
{
    /**
     ...
     * @param RRHHParentesco $datos
     * @return  json(array)
     */

    public function obtener(Request $request, RRHHParentesco $datos)
    {
        $datos = $datos->obtenerParentesco($request);
    ...
    public function obtenerTodas(Request $request, RRHHParentesco $datos)
    {
        $datos = RRHHParentesco::orderby('id_parentesco')->get();
        return response()->json([
            'status' => 'success',
            'result' => $datos,
            'messages' => null
    ...
    {
        $rules = [
            'id_parentesco'=> 'required|integer|min:1'
        ];

        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $datos = RRHHParentesco::find($request->id_parentesco);
