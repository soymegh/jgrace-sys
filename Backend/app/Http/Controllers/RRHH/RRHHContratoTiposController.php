...
class RRHHContratoTiposController extends Controller
{
    /**
     ...
     * @param RRHHContratoTipos $datos
     * @return  json(array)
     */

    public function obtener(Request $request, RRHHContratoTipos $datos)
    {
        $datos = $datos->obtenerContratoTipo($request);
    ...
    public function obtenerTodas(Request $request, RRHHContratoTipos $datos)
    {
        $datos = RRHHContratoTipos::orderby('id_contrato_tipo')->get();
        return response()->json([
            'status' => 'success',
            'result' => $datos,
            'messages' => null
    ...
    public function obtenerContratoTipo(Request $request)
    {
        $rules = [
            'id_contrato_tipo'=> 'required|integer|min:1'
        ];

        $validator = Validator::make($request->all(), $rules);
        if (!$validator->fails()) {
            $datos = RRHHContratoTipos::find($request->id_contrato_tipo);
		  ...
		  return response()->json([
				'status' => 'error',
				'result' => array('id_contrato_tipo'=>["Datos no encontrados"]),
				'messages' => null
			]);
		}
