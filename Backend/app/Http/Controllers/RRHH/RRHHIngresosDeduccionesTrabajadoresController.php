...
class RRHHIngresosDeduccionesTrabajadoresController extends Controller
{
    /**
     ...
     * @param RRHHIngresosDeduccionesTrabajadores $solicitud
     * @return  json(array)
     */

    public function obtener(Request $request, RRHHIngresosDeduccionesTrabajadores $solicitud)
    {
        $solicitud = $solicitud->obtenerIngresoDeduccionTrabajador($request);
    ...
    public function obtenerTodas(Request $request, RRHHIngresosDeduccionesTrabajadores $solicitud)
    {
        $solicitud = RRHHIngresosDeduccionesTrabajadores::orderby('id_cat_ingreso_deduccion_trabajador')->with('asignacionPlanilla','asignacionTrabajador')->get();
    ...
    public function obtenerIngresoDeduccionTrabajador(Request $request)
    {
        $rules = [
        ...
        if (!$validator->fails()) {
            $solicitud = RRHHIngresosDeduccionesTrabajadores::where('id_trabajador',$request->id_trabajador)->first();
		  ...
		  return response()->json([
				'status' => 'error',
				'result' => array('id_cat_ingreso_deduccion_trabajador'=>["Datos no encontrados"]),
				'messages' => null
			]);
		}
    ...
    public function nuevo(Request $request)
    {
        ...
        $ingresos = RRHHIngresosDeducciones::select('id_cat_ingreso_deduccion','cve_ingreso_deduccion','descripcion','codigo',
            DB::raw("CONCAT(descripcion,'(',cve_ingreso_deduccion,')' ) AS Ingreso"))->where('registro_manual',true)->get();
