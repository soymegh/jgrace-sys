...
class BodegasProductos extends Model {
    use HasFactory;
    ...
    */
            ...
            'inventario.v_productos_venta.precio_sugerido', 'inventario.v_productos_venta.precio_distribuidor','inventario.v_productos_venta.costo_promedio','inventario.v_productos_venta.id_tipo_producto']);
        $productos->leftJoin('inventario.v_productos_venta', 'inventario.bodegas_productos.id_producto','inventario.v_productos_venta.id_producto');
        if ((!empty($request->id_bodega))) {
            $productos->Where('inventario.bodegas_productos.id_bodega', $request->id_bodega);
            $productos->Where('inventario.bodegas_productos.cantidad', '>',0);
    ...
    public function productosBodegaRecuperados($request)
    {
        $productos = $this->select(['inventario.v_productos_venta.id_producto','inventario.v_productos_venta.codigo_sistema','inventario.v_productos_venta.unidad_medida','inventario.v_productos_venta.tasa_impuesto','inventario.v_productos_venta.codigo_barra','inventario.v_productos_venta.descripcion','inventario.bodegas_productos.id_bodega_producto'
            ,DB::raw("CONCAT(inventario.v_productos_venta.descripcion,' (',inventario.v_productos_venta.codigo_barra,')') AS text"),
            'inventario.bodegas_productos.cantidad_recuperadas as cantidad_disponible',
            /*  DB::raw("(case when inventario.v_productos_venta.tipo_producto = 3 then
             inventario.obtener_unidades_disponibles(inventario.bodegas_productos.id_bodega,inventario.bodegas_productos.id_producto,1)
          else inventario.bodegas_productos.cantidad end)::INTEGER as cantidad_disponible"),*/
 ...
 */
        ...
        if ((!empty($request->id_bodega))) {
            $productos->Where('inventario.bodegas_productos.id_bodega', $request->id_bodega);
            $productos->Where('inventario.bodegas_productos.cantidad_recuperadas', '>',0);
            /*$productos->WhereRaw("(case when inventario.v_productos_venta.tipo_producto = 3 then
    ...
    public function productosBodegaObsoletos($request)
    {
        $productos = $this->select(['inventario.v_productos_venta.id_producto','inventario.v_productos_venta.codigo_sistema','inventario.v_productos_venta.unidad_medida','inventario.v_productos_venta.tasa_impuesto','inventario.v_productos_venta.codigo_barra','inventario.v_productos_venta.descripcion','inventario.bodegas_productos.id_bodega_producto'
            ...
            'inventario.bodegas_productos.cantidad_obsoletas as cantidad_disponible',

            'inventario.v_productos_venta.precio_sugerido','inventario.v_productos_venta.costo_promedio','inventario.v_productos_venta.id_tipo_producto']);
        $productos->leftJoin('inventario.v_productos_venta', 'inventario.bodegas_productos.id_producto','inventario.v_productos_venta.id_producto');
        if ((!empty($request->id_bodega))) {
            $productos->Where('inventario.bodegas_productos.id_bodega', $request->id_bodega);
            $productos->Where('inventario.bodegas_productos.cantidad_obsoletas', '>',0);
            /*$productos->WhereRaw("(case when inventario.v_productos_venta.tipo_producto = 3 then
    ...
    public function productosBodegaConsignacionCliente($request)
    {
        ...
        $productos->leftJoin('inventario.v_productos_consignados', 'inventario.bodegas_productos.id_bodega_producto','inventario.v_productos_consignados.id_bodega_producto');
        if ((!empty($request->id_cliente))) {
            $productos->Where('inventario.v_productos_consignados.id_cliente', $request->id_cliente);
            $productos->Where('inventario.bodegas_productos.id_bodega', $request->id_bodega);///17 manual
    ...
    {
        return $this->belongsTo('App\Models\InventarioProductosVista','id_producto');
    }

    public function productoVenta()
    {
        return $this->belongsTo('App\Models\InventarioProductosVistaVenta','id_producto');
    }

    public function productoSimple()
    {
        return $this->belongsTo('App\Models\InventarioProductos','id_producto')->select(['id_producto','codigo_sistema','descripcion','id_tipo_producto','condicion']);
    }
