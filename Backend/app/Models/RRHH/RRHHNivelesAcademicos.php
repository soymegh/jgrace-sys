...
class RRHHNivelesAcademicos extends Model
{
    const CREATED_AT = 'f_grabacion';
    const UPDATED_AT = 'f_modificacion';
    protected $table = 'rrhh.niveles_academicos';
    ...
    public function obtenerNivelesAcademicos($request)
    {
        $nivel = $this->select(['*']);
        if (!empty($request->search['field'])) {
            ...
            $nivel->orderBy('id_nivel_academico', 'asc');
        }
